...
class PlayState extends MusicBeatState
{
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			var achieve:String = checkForAchievement(['bees1_nomiss', 'bees2_nomiss', 'bees3_nomiss', 'vicious_50_notes',
				'got_translator', 'vicious_3_notes', 'is_not_christmas', 'mod_100%',
				'tunnel_not_complete', 'who_dis_man?', 'bro_u_sure_mad']);
	...
	private function keyShit():Void
	{
		...
		if (!boyfriend.stunned && generatedMusic)
		{
			...
			if (controlHoldArray.contains(true) && !endingSong) {
				...
				var achieve:String = checkForAchievement(['vicious_50_notes']);
				if (achieve != null) {
					startAchievement(achieve);
				}
				#end
	...
	{	
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				{
					case 'bees1_nomiss' | 'bees2_nomiss' | 'bees3_nomiss' | 'week4_nomiss' | 'week5_nomiss' | 'week6_nomiss' | 'week7_nomiss':
						if(isStoryMode && campaignMisses + songMisses < 1 && CoolUtil.difficultyString() == 'HARD' && storyPlaylist.length <= 1 && !changedDifficulty && !usedPractice)
						{
							...
							{
								case 'week1':
									if(achievementName == 'bees1_nomiss') unlock = true;
									GameJoltAPI.getTrophy(162009);
								case 'week2':
									if(achievementName == 'bees2_nomiss') unlock = true;
									GameJoltAPI.getTrophy(162002);
								case 'week3':
									if(achievementName == 'bees3_nomiss') unlock = true;
									GameJoltAPI.getTrophy(162013);
							}
						}
					case 'ur_bad':
